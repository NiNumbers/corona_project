...
datum <- seq(as.Date("2020/02/26"), Sys.Date(), "days")
corona_kum <- data.frame(datum)
...
#Daten bis ca. 19. März: Tagi, Rest BAG  ----------------------
CHtaegl <- c(1, 7, 4, 9, 6, 15, 16, 32, 26, 94, 54, 73, 52, 120, 136, 213, 267, 234, 291)
CHkum <- cumsum(CHtaegl)
corona_kum$CHkum <- rep(0, dauer)
corona_kum$CHkum[1:19] <- CHkum
corona_kum$CHkum[20:dauer] <- c(2200, 2650, 3028, 3888, 4840, 6113, 7014, 8060, 8836, 9765, 10714, 12161, 13213, 
                                14336, 15475,  16176, 17139, 18267, 19303, 20278, 21100, 21652, 22242, 22789, 23574, 24308, 
                                24900, 25300, 25580, 25834, 26336, 26732, 27078, 27404, 27740, 27944, 28063, 28268, 28496, 
                                28677, 28894, 29061, 29164, 29264, 29407, 29586, 29705, 29817, 29905, 29981, 30009, 30060, 
                                30126, 30207, 30251, 30305, 30344, 30380, 30413, 30463, 30514, 30572, 30587, 30597, 30618,
                                30658, 30694, 30707, 30725, 30736, 30746, 30761, 30776, 30796, 30828, 30845, 30862, 30871,
                                30874, 30893, 30913, 30936, 30956, 30965, 30972, 30988, 31011, 31044, 31063, 31094, 31117,
                                31131, 31146, 31183, 31200, 31217, 31243, 31292, 31310, 31332) 
...
verstorbenCHkum <- c(19, 21, 33, 43, 56, 60, 66, 86, 103, 161, 197, 235, 257, 295, 373, 378, 432, 484, 540, 559, 584, 641, 
                     ...
                     1678, 1678, 1680, 1680, 1680, 1680)

# DE, IT, US, korrelierend auf 1000 Faelle, um 1 Tag verschoben fuer DE, um 8 Tage verschoben für IT, um 2 Tage verschoben für US
...
ITkum <- c(0, 0, 0, 0, 0, 0, 0, 0, 11, 76, 124, 229, 374, 528, 650, 888, 1100, 1700, 2000, 2502, 3089, 3858, 4636,  5883, 
           ...
           236651, 236989, 237290, 237500, 237828, 237950, 238011, 238275, 238499, 238720)
USkum <- c(15, 16, 51, 57, 58, 60, 68, 74, 98, 118, 149, 217, 262, 402, 518, 583, 959, 1300, 1700, 2200, 2700, 3500, 4600, 
           ...
           2074526, 2094069, 2114026, 2137731, 2164071, 2191200, 2222576, 2255119, 2280969, 2312302) 
corona_kum$DEkum <- DEkum[0:dauer]
corona_kum$ITkum <- ITkum[0:dauer] 
corona_kum$USkum <- USkum[0:dauer] 


# ------------- Taegliche Fallzahlen --------
CHtaegl <- 
  corona_taegl <- data.frame(datum)
corona_taegl$day <- corona_kum$day
...
# Landesdataframes machen
corona_DE <- data.frame(seq(as.Date("2020/03/04"), Sys.Date(), "days"))
colnames(corona_DE) <- "datum"
corona_DE$DEkum <- DEkum[9:length(DEkum)]
corona_DE$DEtaegl <- DEtaegl[9:length(DEkum)]

corona_IT <- data.frame(seq(as.Date("2020/02/23"), Sys.Date(), "days"))
colnames(corona_IT) <- "datum"
corona_IT$ITkum <- ITkum[9:length(ITkum)]
corona_IT$ITtaegl <- ITtaegl[9:length(ITkum)]

corona_US <- data.frame(seq(as.Date("2020/02/24"), Sys.Date(), "days"))
colnames(corona_US) <- "datum"
corona_US$USkum <- USkum
corona_US$UStaegl <- UStaegl

# ---------------- save Data ---------
# for shiny app
save(corona_DE, corona_IT, corona_US, corona_taegl, file = "corona_shiny.RData")
# all data
